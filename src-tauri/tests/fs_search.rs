...
use arterm_lib::modules::fs::grep::{fs_glob_inner, fs_grep_inner};
use arterm_lib::modules::fs::search::{fs_list_files_inner, fs_search_inner};
use arterm_lib::modules::fs::tree::{fs_read_dir_inner, list_subdirs_inner, EntryKind};
use arterm_lib::modules::workspace::WorkspaceRegistry;
...
fn glob_finds_files_by_pattern() {
    ...
    let res =
        fs_glob_inner("**/*.rs".into(), fx.root_str(), None, None, &fx.registry).expect("glob");
...
fn glob_truncates_on_limit() {
    ...
    let res =
        fs_glob_inner("*.txt".into(), fx.root_str(), Some(5), None, &fx.registry).expect("glob");
...
fn glob_empty_pattern_errors() {
    let fx = FsFixture::new();
    assert!(fs_glob_inner("".into(), fx.root_str(), None, None, &fx.registry).is_err());
}
...
fn search_substring_matches_filename() {
    ...
    let res = fs_search_inner(fx.root_str(), "main".into(), None, None, None, &fx.registry)
        .expect("search");
    let rels: Vec<&str> = res.hits.iter().map(|h| h.rel.as_str()).collect();
...
fn search_empty_query_returns_empty() {
    let fx = FsFixture::new();
    fx.write("a.txt", "");
    let res = fs_search_inner(fx.root_str(), "   ".into(), None, None, None, &fx.registry)
        .expect("search");
...
fn search_ranks_filename_hits_before_path_hits() {
    ...
    let res = fs_search_inner(fx.root_str(), "zeta".into(), None, None, None, &fx.registry)
        .expect("search");
    let zeta_file = res
        .hits
        .iter()
...
fn list_files_returns_sorted_relative_paths() {
    ...
    let res =
        fs_list_files_inner(fx.root_str(), None, None, None, None, &fx.registry).expect("list");
...
fn list_files_max_depth_clamps() {
    ...
    let res =
        fs_list_files_inner(fx.root_str(), None, Some(1), None, None, &fx.registry).expect("list");
...
fn read_dir_orders_dirs_before_files_then_alpha() {
    ...
    let entries = fs_read_dir_inner(fx.root_str(), false, None, &fx.registry).expect("read_dir");
...
fn read_dir_hides_dotfiles_by_default() {
    ...
    let hidden_off = fs_read_dir_inner(fx.root_str(), false, None, &fx.registry).expect("read_dir");
    let names: Vec<&str> = hidden_off.iter().map(|e| e.name.as_str()).collect();
    assert_eq!(names, vec!["visible.txt"]);

    let hidden_on = fs_read_dir_inner(fx.root_str(), true, None, &fx.registry).expect("read_dir");
...
fn list_subdirs_returns_only_directories() {
    ...
    let dirs = list_subdirs_inner(fx.root_str(), false, None, &fx.registry).expect("list_subdirs");
...
fn list_subdirs_hides_dot_dirs_by_default() {
    ...
    let off = list_subdirs_inner(fx.root_str(), false, None, &fx.registry).expect("list_subdirs");
    assert_eq!(off, vec!["visible"]);

    let on = list_subdirs_inner(fx.root_str(), true, None, &fx.registry).expect("list_subdirs");
