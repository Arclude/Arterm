...
# zsh, so we shadow $? into `_artex_ret`.

{
  _artex_user_zdotdir="${ARTEX_USER_ZDOTDIR:-$HOME}"
  [ -f "$_artex_user_zdotdir/.zshrc" ] && source "$_artex_user_zdotdir/.zshrc"
...
if [[ -z "$__ARTEX_HOOKS_LOADED" ]]; then
  ...
  _artex_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_artex_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _artex_precmd
    add-zsh-hook preexec _artex_preexec
  fi
