# arterm-shell-integration (zshrc)
#
...
# zsh, so we shadow $? into `_arterm_ret`.

{
  _arterm_user_zdotdir="${ARTERM_USER_ZDOTDIR:-$HOME}"
  [ -f "$_arterm_user_zdotdir/.zshrc" ] && source "$_arterm_user_zdotdir/.zshrc"
...
if [[ -z "$__ARTERM_HOOKS_LOADED" ]]; then
  ...
  _arterm_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_arterm_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _arterm_precmd
    add-zsh-hook preexec _arterm_preexec
  fi
