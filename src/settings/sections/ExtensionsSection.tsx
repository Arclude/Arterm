    ...
    <div className="flex flex-col gap-5">
      ...
      {loaded && extensions.length === 0 ? (
        <div className="rounded-lg border border-border/60 px-3 py-6 text-center text-[12px] text-muted-foreground">
          No extensions installed. Click <span className="font-medium">Add sample</span> to
          try one, or drop a package folder into the extensions directory.
        </div>
      ) : null}
        ...
        {extensions.map((ext) => {
          ...
          if (!m) {
            return (
              ...
              >
                <div className="text-[12.5px] font-medium">{ext.folder}</div>
                ...
                <div className="mt-1 flex flex-wrap gap-1.5 text-[10.5px] text-muted-foreground">
                  {ext.themeIds.length > 0 ? (
                    <span className="rounded bg-muted px-1.5 py-0.5">
                      {ext.themeIds.length} theme{ext.themeIds.length > 1 ? "s" : ""}
                    </span>
                  ) : null}
